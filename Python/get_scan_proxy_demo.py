"""
SecuritySurface API - Get Scan Proxy Demo
Demonstrates how to use the API to get available proxy/tunnel servers
"""

import json
from securitysurface_api import SecuritySurfaceAPI


def main():
    """Main function - demonstrates how to use the API to get scan proxy servers"""
    
    # API key
    API_KEY = "HZCeQgAT1xKl3rk0yZokpd8AwXNmSS22fifoLc0OxnLTmzusI9sJJIcRh8Bqdy6m"
    
    # Create API client instance
    api_client = SecuritySurfaceAPI(api_key=API_KEY)
    
    # Get proxy servers
    proxy_data = api_client.get_scan_proxy()
    
    if proxy_data:
        print(json.dumps(proxy_data, indent=2, ensure_ascii=False))
    else:
        print(json.dumps({"error": "Failed to retrieve proxy servers"}, indent=2))


if __name__ == "__main__":
    main()
